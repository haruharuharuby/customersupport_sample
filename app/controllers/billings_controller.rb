class BillingsController < ApplicationController
  before_action :authenticate_csp!
  before_action :set_billing, only: [:show, :edit, :update, :destroy]

  # GET /billings
  # GET /billings.json
  def index
    @billings = current_csp.billings[0].csps
  end
  ...
  def update
    respond_to do |format|
      if @billing.update(billing_params)
        format.html { redirect_to @billing, notice: t("notice.updated") }
  ...
  private
    ...
    def set_billing
      @billing = current_csp.billings.find(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def billing_params
      params.require(:billing).permit(:billing_type,
        nursery_attributes: Nursery.attribute_names.collect { |att| att.to_sym })
    end
end
