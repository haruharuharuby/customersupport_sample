class EstimationsController < ApplicationController
  ...
  # GET /estimations/new
  def new
    @estimation = Estimation.new_with_assoc
  end
  ...
  def search
    ...
    result = Estimation.search uuid

    respond_to do |format|
      if result
        format.json { render json: result, status: :ok }
      else
        format.json { render json: {error: t("errors.estimation_not_found")}, status: :unprocessable_entity }
      end
    end
  end
